...
export default function AfspraakToevoegen() {
  ...
  if (!session) {
    Alert.alert("Fout", "Geen geldige sessie");
    return null;
  }

  const { date, dateInput } = params;
    ...
    try {
      const res = await fetch("http://10.2.160.216:8000/user/afspraken/", {
        ...
        body: JSON.stringify({
          ...
          date: `${dateInput} ${uur}`,
          dementegebruikerId: session.dementgebruikerid,
          extra,
        }),
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput style={styles.input} placeholder="Titel" value={titel} onChangeText={setTitel} />
      <TextInput style={styles.input} placeholder="Locatie" value={locatie} onChangeText={setLocatie} />
      <TextInput
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: "#999", borderRadius: 8, padding: 12, marginBottom: 15 },
  textarea: { height: 80 },
  buttons: { flexDirection: "row", justifyContent: "space-between" },
  cancel: { backgroundColor: "#ccc", padding: 14, borderRadius: 8, flex: 1, marginRight: 10, alignItems: "center" },
  save: { backgroundColor: "#4CAF50", padding: 14, borderRadius: 8, flex: 1, alignItems: "center" },
  pickerWrapper: { borderWidth: 1, borderColor: "#999", borderRadius: 8, marginBottom: 15, overflow: "hidden" },
});
